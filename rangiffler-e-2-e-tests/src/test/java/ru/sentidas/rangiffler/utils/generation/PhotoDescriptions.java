...
/**
 * Класс: PhotoDescriptions
 * Назначение: хранит локализованные фразы для описаний фото и выдает случайную фразу.
 * Ключи мапы — (ru, en, es, zh-cn, pt-br, pt-pt, fr, de, hi, bn, ar).
 */
public final class PhotoDescriptions {
    ...
    private static final Map<String, List<String>> DESCRIPTIONS = Map.ofEntries(
            ...
            Map.entry("pt-pt", List.of(
                    ...
                    "Silêncio mais bonito que música",
                    "Falésias, ondas e coragem",
                    "Uma vila donde cada casa tem alma",
                    "A manhã tem gosto de esperança",
                    "Cansaço doce que abraça",
                    "Um ponto no mapa, uma alegria gigante",
                    "Que esta luz siga comigo"
            ...
            Map.entry("de", List.of(
                    ...
                    "Steingassen и frisches Brot — wie im Film",
    ...
    /**
     * Возвращает случайную фразу по локали (любой регистр/форма на входе).
     * Неизвестная локаль → default на en; для "pt-*" - "pt-br".
     */
    public static String pickRandom(String localeTag) {
        String tag = LocaleUtil.normalize(localeTag);
        List<String> bag = DESCRIPTIONS.get(tag);
        if (bag == null) {
            if (tag.startsWith("pt")) {
                bag = DESCRIPTIONS.getOrDefault("pt-br", DESCRIPTIONS.get("en"));
            } else {
                bag = DESCRIPTIONS.get("en");
            }
        }
        int i = ThreadLocalRandom.current().nextInt(bag.size());
        return bag.get(i);
    }
}
