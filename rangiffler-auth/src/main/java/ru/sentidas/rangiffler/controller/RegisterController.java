package ru.sentidas.rangiffler.controller;
...
public class RegisterController {
  ...
  public String getRegisterPage(@Nonnull Model model) {
    ...
    model.addAttribute(MODEL_FRONT_URI_ATTR, rangifflerFrontUri + "/my-travels");
    ...
    if (!errors.hasErrors()) {
      ...
      try {
        ...
        addErrorToRegistrationModel(
            registrationModel,
            model,
            "username", "Username `" + registrationModel.username() + "` already exists user"
        );
      }
    } else {
      response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
