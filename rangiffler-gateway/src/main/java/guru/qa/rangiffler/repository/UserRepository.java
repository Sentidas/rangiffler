...
public interface UserRepository extends JpaRepository<UserEntity, UUID> {
    ...
    @Query("select u from UserEntity u join FriendshipEntity f on u = f.addressee" +
            " where f.status = guru.qa.rangiffler.model.FriendshipStatus.ACCEPTED and f.requester = :requester")
    ...
    @Query("select u from UserEntity u join FriendshipEntity f on u = f.addressee" +
            " where f.status = guru.qa.rangiffler.model.FriendshipStatus.PENDING and f.requester = :requester")
    ...
    @Query("select u from UserEntity u join FriendshipEntity f on u = f.requester" +
            " where f.status = guru.qa.rangiffler.model.FriendshipStatus.PENDING and f.addressee = :addressee")
